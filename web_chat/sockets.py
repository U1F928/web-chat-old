...
@socketio.on("join")
def join(room_name):
    if web_chat.models.Room.query.get(room_name) == None:
        web_chat.models.Room.create(room_name)
...
def handle_newest_id_request(room_name):
    room = web_chat.models.Room.query.get(room_name)
    if room == None:
...
def handle_comments_request(comments_request):
    ...
    comments = web_chat.models.Comment.query.filter(
        web_chat.models.Comment.room_name == room_name,
        web_chat.models.Comment.in_room_id <= biggest_id,
        web_chat.models.Comment.in_room_id >= smallest_id,
    )
...
def handle_new_comment(comment):
    comment = web_chat.models.Comment.create(comment)
    flask_socketio.emit(
